...
Данный алгоритм был предложен в 1994 Филиппом Гейджем в статье `“A New Algorithm for Data Compression” <https://psv4.vkuserphoto.ru/s/v1/d2/tdhBMA8VdWMafM_RkqNDWkUoCJ9EMOqG6oNLkM85cmUz1V0nBpKmx73r0KRXDFbUJH5LZABX5wMlWzz8VvKoQImcWxgOAJt5P0agrS7Tpkm9NZZj1oC__3Al-33xK9qgsatvBxHXRc0W/Philip_Gage_-_Novy_algoritm_szhatia_dannykh.pdf>`__.
...
.. warning:: **Знаки препинания как часть слова**

             Обратите внимание, что в рамках данной лабораторной работы текст
             разделяется на слова исключительно по пробельным символам (методом ``.split()``).
             Поэтому знаки препинания (например, запятые в ``"far,"`` или ``"old,"``)
             не удаляются и технически **становятся частью слова**.

             Алгоритм BPE воспринимает их как обычные символы, поэтому в процессе
             обучения могут возникать токены, содержащие пунктуацию (например, ``',</s>'``).
             В реальных задачах текст обычно проходит предварительную токенизацию
             или очистку, но здесь мы оставляем их для демонстрации базовой работы алгоритма.
...
Результатом работы алгоритма BPE становится словарь токенов (vocabulary) заданного
размера (базовый алфавит символов + количество слияний ``num_merges``).
Для нашего примера итоговое разбиение слов на токены будет выглядеть следующим образом:
...
.. note:: **Зачем нужен** ``None`` **для границ слова?**

          В качестве самого слова (``raw_word``) всегда передается обычная строка.
          Значение ``None`` используется только для параметров начала (``start_of_word``)
          или конца (``end_of_word``) слова.

          Это флаг: если параметр равен ``None``, алгоритм понимает,
          что этот технический токен **добавлять к слову не нужно**.
          Например, в классическом BPE на Шаге 2 токен начала слова не используется,
          соответственно в ``start_of_word`` передается ``None``.
...
.. warning:: **Ошибочное появление пустого токена (''):**

             В итоговом словаре токенов **не должно быть пустых строк** (``''``).
             Если при генерации словаря или выводе программы вы видите пустой токен,
             это указывает на ошибку в реализации функции ``merge_tokens``.

             Обычно это происходит по причине некорректного вычисления индексов
             при замене пары токенов на один объединенный в списке букв слова.
             Убедитесь, что ваша функция слияния не оставляет «пустот» внутри кортежей слов.
...
.. note:: **Что является результатом выполнения этапа на 6 баллов:**

          Результатом этого этапа является обученный BPE токенизатор.
          Алгоритм последовательно объединил наиболее часто встречающиеся
          последовательности символов в укрупненные единицы - токены. Вы
          должны увидеть, что вместо посимвольного разбиения в словах
          настоящего русского текста выделились повторяющиеся морфемы,
          корни и окончания (например, ``"то"``, ``"го"``, ``"ни"``). Это
          показывает, что токенизатор успешно нашел закономерности.
...
Модели машинного перевода не умеют работать с сырым текстом напрямую,
они принимают только числа. Поэтому каждую текстовую последовательность
необходимо преобразовать в числовой вектор.
Для этого мы строим словарь (vocabulary), где каждому уникальному токену
и символу присваивается свой числовой идентификатор (ID).
...
.. important:: Выполнение Шага 7 соответствует 6 баллам.
...
.. note:: **Разбор жадного алгоритма сегментации слова:**

          Поиск токенов в слове происходит не слева направо по буквам,
          а **по убыванию длины токенов** в словаре.

          1. Перед началом работы функции все токены из предоставленного
             словаря сортируются строго по длине (от самых длинных к коротким).
          2. Программа берет самый длинный доступный токен и проверяет,
             есть ли он внутри кодируемого слова.
          3. Если токен найден, он заменяет собой буквы слова.
          4. Программа переходит к следующим, более коротким токенам.

          **Почему выбирается "ord", а не "wo"?**
          Токен ``'ord'`` имеет длину 3, а токен ``'wo'`` длину 2.
          Так как мы ищем от самых длинных к коротким, токен ``'ord'``
          проверяется первым, находит совпадение в слове ``"word"``
          и поглощает букву ``'o'``. После этого токен ``'wo'`` применить
          уже невозможно. Верный результат: ``['w', 'ord']``.
...
.. important:: Выполнение Шага 10 соответствует 8 баллам.
...
.. important:: **Символьный или словесный BLEU?**

               В рамках данной лабораторной работы функции ``collect_ngrams``
               и ``calculate_bleu`` работают **на уровне отдельных символов (букв)**
               поверх единой сырой текстовой строки (включая знаки препинания и пробелы),
               а не на уровне разделенных слов.

Например, при вызове функции ``collect_ngrams`` с текстовой последовательностью
(строкой) ``"мыть"`` и порядком слияния 3, мы работаем с символами и ожидаем
получить результат в виде списка кортежей: ``[('м', 'ы', 'т'), ('ы', 'т', 'ь')]``.
...
:py:func:`lab_2_tokenize_by_bpe.main.calculate_geo_mean`.
...
расположен в файле ``assets/ru_raw.txt``.
...
.. note:: **Зачем нужен токен начала слова** (например, ``\u2581``) **?**

          В продвинутых реализациях BPE пробелы между словами
          не удаляются, а заменяются специальным символом начала слова — ``\u2581``.

          Этот токен создается функцией ``prepare_word`` и приклеивается **строго
          в начало** каждого слова. Он выполняет две важнейшие задачи:

          1. Позволяет алгоритму BPE на этапе обучения склеивать маркер пробела
             с первыми буквами слова, если это сочетание встречается очень часто.
          2. Позволяет функции ``decode`` безошибочно восстанавливать исходный
             текст. При декодировании все технические токены начала слова ``\u2581``
             просто заменяются на обычные пробелы, возвращая тексту человеческий вид.

Чтобы убедиться, что файл удалось закодировать верным образом,
можно сравнить его с закодированной версией текста из файла
``assets/ru_encoded.txt``.
Закодированные предсказания модели (то есть предсказания в том виде, в
котором их вернула модель) можно найти в файле
``assets/en_encoded.txt``.
...
.. note:: **Какова роль нейросетевой модели в этом задании?**

          В рамках этой задачи мы симулируем работу системы машинного перевода.
          Нейросетевая модель уже перевела текст за нас. Закодированные предсказания
          модели на английском языке находятся в файле ``en_encoded.txt``.
          Текст в нём **уже переведен**, но представлен числовыми ID.

          Ваша задача: загрузить готовый словарь ``vocab.json``, декодировать
          числовой английский перевод в понятные слова и оценить качество
          работы нейросетевой модели с помощью метрики BLEU относительно идеального
          человеческого перевода из файла ``en_raw.txt``.
